        ...
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Finalize a habilitação da autenticação de dois fatores.') }}
                @else
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('Para finalizar a habilitação da autenticação de dois fatores, escaneie o QR code a seguir usando o aplicativo autenticador do seu telefone ou insira a chave de configuração e forneça o código OTP gerado.') }}
                        @else
                            {{ __('A autenticação de dois fatores está agora habilitada. Escaneie o QR code a seguir usando o aplicativo autenticador do seu telefone ou insira a chave de configuração.') }}
                        @endif
                    </p>
                </div>
