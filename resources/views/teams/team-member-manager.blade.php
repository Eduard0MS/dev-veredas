<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('Adicionar membro ao projeto') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Adicione um novo membro ao seu projeto, permitindo que ele colabore com você.') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Forneça o endereço de e-mail da pessoa que você gostaria de adicionar a este projeto.') }}
                        </div>
                    </div>
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('Convites para o projeto pendentes!') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Essas pessoas foram convidadas para o seu projeto e receberam um e-mail de convite. Eles podem ingressar no projeto aceitando o convite por e-mail.') }}
                </x-slot>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('Membros do Projeto') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Todas as pessoas que fazem parte deste Projeto.') }}
                </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('Tem certeza de que gostaria de sair deste projeto?') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('Remover membro do projeto') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Tem certeza de que deseja remover esta pessoa do projeto?') }}
        </x-slot>
